...
namespace SimpleSongsPlayer.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        public List<LyricBlock> AlllLyricBlocks { get; private set; }

        public async Task ScanFolders(IEnumerable<StorageFolder> folders)
        {
            AllSongs = new List<Song>();
            AlllLyricBlocks = new List<LyricBlock>();
            ...
            foreach (var lyricsFile in lyricsFiles)
                AlllLyricBlocks.Add(new LyricBlock(lyricsFile.DisplayName, await FileReader.ReadFileAsync(lyricsFile)));
        }
    }
}